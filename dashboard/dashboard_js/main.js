import { app, db, auth, collection, getDocs, doc, getDoc, updateDoc, deleteDoc, addDoc, query, where, orderBy, Timestamp, writeBatch, deleteUser } from './firebaseConfig.js';
...
function AddItemToTable(name, ps, sn, docId, stdNo, tbody) {
    ...
    trow.innerHTML = `
        ...
        <td>
            ...
            <button class="action-buttonV" data-docid="${docId}">v</button>
            <button class="action-buttonU" data-docid="${docId}">u</button>
            <button class="action-buttonD" data-docid="${docId}">d</button>
            <div>
        </td>
    `;
...
async function deleteDocumentAndAuth(docId) {
    ...
    try {
        ...
        const response = await fetch(`http://localhost:3000/deleteUser/${docId}`, {
            method: "DELETE",
        });
